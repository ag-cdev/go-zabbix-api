...
// TemplatesMassAdd links templates to host groups
// https://www.zabbix.com/documentation/7.0/en/manual/api/reference/template/massupdate
func (api *API) TemplatesMassAdd(templates TemplateIDs, hostGroups HostGroupIDs) error {
	if len(templates) == 0 || len(hostGroups) == 0 {
		return nil
	}
	...
	_, err := api.CallWithError("template.massupdate", params)
	return err
}
